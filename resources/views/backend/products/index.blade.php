...
<div class="card">
    <div class="card-header">
        <div class="card-title flex-column">
            <h3 class="fw-boldest m-0">Product List</h3>
        </div>
            ...
            <tbody class="fs-6 fw-boldest">
              @foreach ($products as $product)
                <tr>
                    <td>
                        <div class="d-flex">
                            <div class="symbol symbol-75px me-6 bg-light">
                                <img src="{{asset('/storage/'.$product->image->attachmentable_image)}}"  alt="image" >
                            </div>
                        </div>
                    </td>
                    <td>{{$product->name }}</td>
                    ...
                    <td class="text-end">
                        <a href="{{route('webadmin.products.show',$product->id) }}" class="btn btn-light btn-sm me-2">View</a>
                        <a href="{{route('webadmin.products.edit',$product->id)}}" class="btn btn-light btn-sm">Edit</a>
                       <form method="post" class="delete-form" data-route="{{route('webadmin.products.destroy',$product->id)}}">
                            @method('delete')
